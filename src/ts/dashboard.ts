...
function displayAnalysis(data: any) {
  ...
  const incomeElement: HTMLElement | null = <HTMLElement>(
    document.getElementById("income")
  );
  incomeElement.innerHTML = daIncome + " €";
  const productsElement = <HTMLElement>document.getElementById("products");
  productsElement.innerHTML = daProducts;
  const productsTitleElement = <HTMLElement>(
    document.getElementById("products-title")
  );
  productsTitleElement.innerHTML = daProducts <= 1 ? "Produkt" : "Produkts";
  const studentsElement = <HTMLElement>document.getElementById("students");
  ...
  const endDateElement = <HTMLElement>document.getElementById("end-date");
  endDateElement.innerHTML = minMax.dateMax;
}

function pieChart(date: any) {
...
function areaChart(res: any) {
  const dataAnalysis = new DA(res);
  dataAnalysis.setRowName("Formatted Date", "date");
  let data: any[] = dataAnalysis.getOption().repeated;
  let date: any[] = dataAnalysis.getOption().dateYYYYMMDD;
...
function barChart(res: any) {

  const dataAnalysis = new DA(res);
  dataAnalysis.setRowName("Formatted Date", "date");
  let data: any[] = dataAnalysis.getOption().repeatedWeekday;
