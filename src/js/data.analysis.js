...
module.exports = class DataAnalysis {
    ...
    setRowName(itemRowName, type) {
        try {
            Array.from(this.data).forEach((row) => {
                switch (type) {
                    case undefined:
    ...
    analysisDate() {
        try {
            ...
            //YYYY-MM-DD
            let sortedDate = this.items.sort((a, b) => new Moment(a).format("YYYYMMDD") - new Moment(b).format("YYYYMMDD"));
            sortedDate.forEach((sDate) => {
            ...
            this.allDate.forEach((aDate) => {
                let getDay = new Date(aDate).getDay();
                day[getDay].repeated = day[getDay].repeated + 1;
            });
            day.push(day.splice(0, 1)[0]);
            this.weekDay = day.map((a) => a.repeated);
            //MIN and MAX
            this.dateMin = new Date(Math.min.apply(null, this.allDate)).toLocaleString("de-DE", {
                weekday: "long",
                day: "2-digit",
                month: "2-digit",
                year: "numeric",
            });
            this.dateMax = new Date(Math.max.apply(null, this.allDate)).toLocaleString("de-DE", {
                weekday: "long",
                day: "2-digit",
                month: "2-digit",
                year: "numeric",
    ...
    comparisonVsDate(itemRowNameComparison, itemRowNameDate) { }
};
